...
06.01.2017 - NBO: Logikfehler im Skript (ForEachschleife)
...
function translateSID {
   	Param ([string]$VHDPath)
    $VHDPath -replace ".{5}$" # entferne .vdhx von String (letzte 5 characters)
    $objSID = New-Object System.Security Principal.SecurityIdentifier($VHDPath)
...
## Erweiterungsvorgang, wird gestartet wenn die vDisk nicht genutzt wird (=$true)
	If($state -eq $true)
		{
		    # VHD Aktuelle max. Grösse Output
		    $vhds = Get-VHD -Path $_
    ...
    # Get User from SID
    translateSID -VHDPath $_

    # Zusammenstellen des Reportings    
    $free = ($vhds.Size/1KB)-($file.length/1KB)
	$report += New-Object psobject -Property @{"Disk"=$($file);"Shared"=$($file);"Resize"=$($resize);"Frei (MB)"=$($free/1024);"Belegt (MB)"=$($file.length/1KB/1024);"Gesamt (MB)"=$($vhds.Size/1KB/1024)}
...
$report | select "Disk", "Resize", "Frei (MB)", "Belegt (MB)", "Gesamt (MB)" | Format-Table –AutoSize
# Table mit allen VHDs in Log schreiben
$report | select "Disk", "Resize", "Frei (MB)", "Belegt (MB)", "Gesamt (MB)" | Format-Table –AutoSize > ".\Diskreport_$now.log"
$i = 0
